...
def apply_AlphaEdit_to_model(
    ...
    for i, layer in enumerate(hparams.layers):
        ...
        layer_ks.to(model.device)
        resid.to(model.device)
        upd_matrix = torch.linalg.solve(
                P[i,:,:].cuda().to(model.device) @ (layer_ks @ layer_ks.T + cache_c[i,:,:].cuda().to(model.device)) + hparams.L2*torch.eye(layer_ks.shape[0], dtype=torch.float,device=model.device), P[i,:,:].cuda().to(model.device) @ layer_ks @ resid.T
        )
